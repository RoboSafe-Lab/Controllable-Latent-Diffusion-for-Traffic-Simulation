...
from data.load_data import create_dataset, create_dataloader
...
def main():
    ...
    parser.add_argument('--model_config', type=str, default='configs/models/vae_configs.yaml', help='模型配置文件路径')
    ...
    # 创建数据集和数据加载器
    train_dataset = create_dataset(dataset_config)
    train_dataloader = create_dataloader(train_dataset, dataset_config)
    
    # 创建验证数据集和数据加载器
    val_dataset = create_dataset(dataset_config, split='val')
    val_dataloader = create_dataloader(val_dataset, dataset_config)
